...
wrong-projectile-model probe is intentionally a high-scoring floor-fail probe:
it proves a model-only defect cannot receive 100/100 or `passed: true` even
when the rest of the behavior is strong. Record every probe run in the Observed
column and keep its score JSON as curated evidence under `evaluation/evidence/`.
...
| Wrong projectile model on otherwise complete behavior | 95-98 | `98/100` on 2026-07-04, `passed: false`, `visual_audio_polish` is `3/5` and fails the visual floor (`evaluation/evidence/calibration-20260704-probe-wrong-projectile-model-score.json`) | `Thrown grenade model` detail stays `0/2` with placeholder primitive evidence; score cannot remain `100/100`, and `visual_audio_polish >= 4` blocks `passed`. |
...
The fast probe pass now has eight observed fake candidates with committed score
JSON evidence. Seven remain below the numeric `score >= 85` pass line; the
wrong-projectile-model probe deliberately stays high-scoring while failing the
visual floor. Deferred rows are intentionally documented as overlapping
validity work rather than silent passes; run them later if the verifier scoring
changes or if the final reviewer asks for exhaustive per-row evidence. The
observed global-sweep score sits 7 points below the 85 pass line, so re-run that
probe first whenever explosion scoring changes; re-run the wrong-model probe
whenever visual scoring changes.
