...
from an older presentation calibration point; it is not an active probe and
should not be rerun unless a future rubric explicitly restores it.
...
(`trajectory_preview >= 9`, `projectile_physics >= 8`,
`explosion_gameplay >= 10`, `stability_repeatability >= 5`) hard-block `passed`
...
There is no `visual_audio_polish` pass floor; rendered presentation defects
now affect the formal score through the 20-point `visual_audio_polish` category.

The observed 2026-07-06 scores below were recorded before render-backed
presentation evidence was folded into the formal score. Re-run the reference,
ablated task, and seven active probes with the render-capable formal grader
before publishing refreshed score bands.
...
| --- | ---: | --- | --- |
| HUD-only weapon switch, no projectile | 10-30 | Previous rubric: `17/100` on 2026-07-06, `passed: false`, core floors and stability floor fail (`evaluation/evidence/calibration-20260706-probe-hud-only-score.json`) | `projectile_physics`, calibration, `explosion_gameplay`, and `stability_repeatability` stay low. |
...
score JSON remains in `evaluation/evidence/` as older presentation evidence,
but it is no longer counted in the active probe set. Because render-backed
presentation scoring changes the formal score, refresh all seven active probe
rows before treating the matrix as a current calibration table. Re-run the
global-sweep probe first whenever explosion scoring changes; re-run the
visual-only and damage/trajectory probes plus the asset-quality and rendered
footprint tests whenever visual scoring changes.
